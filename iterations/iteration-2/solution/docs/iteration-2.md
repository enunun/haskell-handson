...
Iteration 1からの変更点は次のとおり．

- Context・Container：説明を「支出の明細と合計を表示する」に直した．
- Component：`Kakeibo.Entry`を足した．`Kakeibo.App`は`parseEntry`と`formatEntry`を，`Kakeibo.Entry`は`formatEntry`の中で`formatYen`を使うので，`Kakeibo.Entry`から`Kakeibo.Money`への矢印も要る．
- Code：
  - 「データ型」の節を足し，`Category`(直和型)と`Entry`(レコード)を描いた．
  - 型と関数の流れを，`[Entry]`から明細の行と合計の行に分かれ，`++`でつながる形にした．
  - `parseEntry`の中で，`break`の結果を`:`があるかどうかで分ける流れを別の図にした．

`Entry`を中心に据えると，引数を`Entry`に変える部分(`parseEntry`)と，`Entry`から行を作る部分(`formatEntry`，合計)に分かれることが図から読み取れる．
テストリストの単体テストも，この2つの部分ごとにまとまっている．
...
5. 解答例は設計書どおり．`parseEntry`は，最初は`case`を使わない形で書き，テストを足しながら，設計書に描いた場合分けの形に近づけた．
