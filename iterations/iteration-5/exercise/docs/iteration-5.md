...
- `03-component.md`に`Kakeibo.Display`を足す．`Display`のインスタンスを定義するモジュールと，`display`を呼ぶモジュールから，矢印を引く．
- `Data.Map.Strict`のように，ほかのパッケージのモジュールを使うときは，境界の外に`Component_Ext`で描く．
- `04-code.md`のデータ型の図に，`newtype Yen`と，型クラス(`Display`・`Semigroup`・`Monoid`・`Ord`など)と，インスタンスの関係を描く．インスタンスごとの定義(`<>`は足し算，など)は表にする．
- 型と関数の流れで，金額の型を`Yen`にし，表示の関数を`display`にする．
- `summarize`の中で，`Map`を作ってから金額の大きい順に並べるまでの流れを描き直す．
...
6. 設計書と実装を見比べる．実装してみて，設計書と違う形になったところはあるか．あれば，設計書を実装に合わせて直す．
