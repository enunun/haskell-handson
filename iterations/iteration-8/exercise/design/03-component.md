...
C4Component
  ...
  Container_Boundary(cli, "kakeibo実行ファイル") {
    Boundary(io, "IOを行う部分") {
      ...
      Component(app, "Kakeibo.App", "src/Kakeibo/App.hs", "サブコマンドを実行する．年月の指定があれば支出を絞り込む")
    ...
    Boundary(pure, "純粋な部分") {
      ...
      Component(date, "Kakeibo.Date", "src/Kakeibo/Date.hs", "日付と年月の型と，その読み取り・表示")
  ...
  Rel(date, display, "instance Display")
```

- `IO`を行うのは`Main`，`Kakeibo.App`の`run`，`Kakeibo.Storage`の`loadEntries`と`appendEntries`である．
- 引数の読み取り，データファイルの行の変換，日付の読み取り，表示する行の組み立ては，すべて純粋な関数にし，単体テストで確かめる．
